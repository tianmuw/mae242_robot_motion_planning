...
import rl_files.framework as framework
import rl_files.utils as utils
...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if np.random.random() < epsilon:
        ...
        object_index = OBJECTS.index(chosen_object)
    else:
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        next_r_desc, next_q_desc, reward, terminal = framework.step_game(current_room_desc, current_quest_desc,
                                                                         action_index, object_index)
        next_r_index = dict_room_desc[next_r_desc]
        next_q_index = dict_quest_desc[next_q_desc]
        if for_training:
            # update Q-function.
            tabular_q_learning(q_func, current_room_index, current_quest_index, action_index,
                               object_index, reward, next_r_index, next_q_index,
                               terminal)
            pass
        ...
        if not for_training:
            ...
            # epi_reward = pow(GAMMA, step) * reward
            step += 1
            pass

        # prepare next step
        current_room_index = next_r_index
        current_quest_index = next_q_index
        current_room_desc = next_r_desc
        current_quest_desc = next_q_desc
